const kofiWidgetOverlayConfig={
    ...
    'floating-chat.cssId':'','floating-chat.notice.text':'ko-fi.com/%HANDLE%',
...
var kofiWidgetOverlayFloatingChatBuilder=kofiWidgetOverlayFloatingChatBuilder||function(config,_utils){
    ...
    function insertPopupHtmlIntoBody(donateButton,selectors,parentElementId){
        ...
        closer.addEventListener('click',function(event){closePopup(popup,donateButton);});
    ...
    function toggleKofiIframe(iframeId,state,donateButton,selectors,heightLimits){
        ...
        setTimeout(function(){closeButtonActionBlocked=false;},1000);}
    };
...
var kofiWidgetOverlayUtilities=kofiWidgetOverlayUtilities||function(){
    ...
    const debounce=function(debounceRef,callback){
        if(debounceRef===null){
            debounceRef=setTimeout(function(){clearTimeout(debounceRef);debounceRef=null;callback();},100);
        }
    };
    ...
    const loadStyleSheet=function(styleSheetHref,targetDocument){
        ...
        if(styleSheet.length===0){
            ...
            sslink.href=styleSheetHref;sslink.rel='stylesheet';
...
var kofiWidgetOverlay=kofiWidgetOverlay||(function(){
    ...
    const setConfigDefaults=function(config,widgetType,pId,instanceId){
        ...
        config[widgetType+'.cssId']=config[widgetType+'.cssId']!==undefined&&config[widgetType+'.cssId']!==''?config[widgetType+'.cssId']:instanceId;config[widgetType+'.stylesheets']=config[widgetType+'.stylesheets']!==undefined?config[widgetType+'.stylesheets']:'["https://fonts.googleapis.com/css?family=Nunito:400,700,800&display=swap"]';
        return config;
    }
